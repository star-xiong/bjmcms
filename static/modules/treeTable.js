layui.define(['jquery'], function(exports) {
	...
	tree.prototype.template = function(e) {
		var t = this,
			...
			checkbox = e.is_checkbox ? '<div class="layui-unselect layui-form-checkbox cbx" lay-skin="primary" style="padding-left: 18px; margin-left: 5px;"><i class="layui-icon layui-icon-ok"></i></div>' : '',
			checked = checkbox ? checkbox.replace('cbx', 'cbx layui-form-checked') : '',
			tcheckbox = e.is_checkbox ? '<div class="layui-unselect layui-form-checkbox cdx" lay-skin="primary" style="padding-left: 18px; margin-left: 5px;"><i class="layui-icon layui-icon-ok"></i></div>' : '',
			tchecked = tcheckbox ? tcheckbox.replace('cdx', 'cbx layui-form-checked') : '',
			thead = tcheckbox && '<th style="width:28px;">' + (o.inArray(e.top_value, e.checked.data) > -1 ? tchecked : tcheckbox) + '</th>';
		o.each(t.data(e, e.data), function(idx, item) {
			...
			o.each(e.cols, function(index, obj) {
				...
				// 指定列加入开启、关闭小图标
				var icon = (obj.key == e.icon_key) ? '<em class="layui-tree-iconClick layui-tree-icon"><i class="' + (t.cache(e, item[e.primary_key]) ? e.icon.open : e.icon.close) + '"></i></em>' : '<span></span>';
				// 指定列小图标按照层级向后位移
		...
		o(e.elem).html(table).off('click', click_btn).on('click', click_btn, function() {
		...
		}).off('click', '.cdx').on('click', '.cdx', function() {
			...
			layui.event.call(this, MOD_NAME, 'tree(box)', {
				elem: this,
				item: pid === undefined ? {} : e.childs[id],
				table: e.elem,
	...
	tree.prototype.parents_checkbox = function(e, id) {
		var po = o(e.elem).find('.cbx'),
			co = o(e.elem).find('.cdx'),
			len = o(e.elem).find('.cbx.layui-form-checked').length;
		if(po.length == len) {
			co.addClass('layui-form-checked');
		}
		else{
			co.removeClass('layui-form-checked');
		}
	};
	//全选反选
	...
	// 获取已选值排序
	tree.prototype.rank = function(e) {
		var ids = [];
		o(e.elem).find('.cbx.layui-form-checked').each(function() {
			var id = o(this).parents('[data-id]').data('id');
			var rank = o(this).parents('[data-id]').find('.rank').val();
			ids.push(id+'||'+rank);
		})
		return ids;
	};
